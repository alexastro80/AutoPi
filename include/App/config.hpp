...
public:
	...
	inline QColor Color() {
		return QColor(darkMode ? ColorDark : ColorLight);
	}
	...
	QString LauncherApp;
	bool MouseActive;
	inline void SetShortcut(QString id, QString _short) {
		shortcuts[id] = _short;
	}
	inline QString GetShortcut(QString id) {
		return shortcuts[id];
	}

	inline QString QuickView() {
		return quickView;
	}
	...
	inline QMap<QString, QWidget*> GetQuickViews() {
		return quickViews;
	}
	inline QWidget* GetQuickView(QString name) {
		return quickViews[name];
	}
	inline void AddQuickView(QString name, QWidget *view) {
		quickViews[name] = view;
	}

	double Scale;

	inline bool Page(QWidget *page) {
		return pages.value(page->objectName(), true);
	}
...
public slots:
	void setVideoResolution(std::string resolution) {
		if (resolution == "480")
			openautoConfig->setVideoResolution(
					aasdk::proto::enums::VideoResolution_Enum__480p);

		else if (resolution == "720")
			openautoConfig->setVideoResolution(
					aasdk::proto::enums::VideoResolution_Enum__720p);
		else if (resolution == "1080")
			openautoConfig->setVideoResolution(
					aasdk::proto::enums::VideoResolution_Enum__1080p);
	}
	void setVideoFPS(std::string resolution) {
		if (resolution == "480")
			openautoConfig->setVideoFPS(
					aasdk::proto::enums::VideoFPS_Enum__30);
