...
Theme::Theme() :
		QObject(qApp), palette() {
	QFontDatabase::addApplicationFont(
			":/fonts/Nasalization/nasalization-rg.ttf");
...
QString Theme::scaleStylesheet(QString stylesheet) {
	...
	while (i.hasNext()) {
		...
		if (match.hasMatch()) {
			...
			stylesheet.replace(match.captured(),
					QString("%1px").arg(scaled_px));
		}
	}
...
QIcon Theme::themedButtonIcon(QIcon icon, QAbstractButton *button) {
	...
	QBitmap alt_icon_mask;
	{
		...
		if (!alt_icon.isNull())
			alt_icon_mask = alt_icon.pixmap(size).createMaskFromColor(
					Qt::transparent);
	}
	...
	normal_on.fill(
			!button->property("page").isNull() ? accent_color : base_color);
	...
	QPixmap disabled_off(size);
	{
		...
		disabled_on.setMask(
				!alt_icon_mask.isNull() ? alt_icon_mask : icon_mask);
...
QIcon Theme::MakeButtonIcon(QString name, QPushButton *button,
		QString alt_name) {
	if (!alt_name.isNull())
		button->setProperty("alt_icon",
				QVariant::fromValue(
						QIcon(QString(":/icons/%1.svg").arg(alt_name))));
	QString path;
	QFileInfo checkIcon;
	if (Mode) {
...
void Theme::Update(bool mode) {
	if (mode != Mode) {
		setPalette();
		qApp->setStyleSheet(
				scaleStylesheet(stylesheets[Mode ? "dark" : "light"]));

		for (QWidget *widget : qApp->allWidgets()) {
			QAbstractButton *button = qobject_cast<QAbstractButton*>(widget);
			if ((button != nullptr)
					&& !button->property("themed_icon").isNull())
				button->setIcon(themedButtonIcon(button->icon(), button));
		}

		Mode = mode;
		std::cout << "Mode updated: " << (Mode) ? "TRUE\n" : "FALSE\n";
